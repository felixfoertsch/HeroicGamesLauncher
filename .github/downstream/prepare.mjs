...
export const releaseTagPattern = /^v?\d+\.\d+\.\d+-\d{4}\.\d{2}\.\d{2}\.[1-9]\d*$/
...
export function git(repo, ...args) {
  return run('git', ['-C', repo, '-c', 'user.name=Heroic downstream automation',
...
export function nextVersion(upstreamTag, date, tags) {
  if (!stableTagPattern.test(upstreamTag)) throw new Error('Expected a stable upstream version tag')
  ...
  const used = tags.filter((tag) => tag.startsWith(prefix))
    .map((tag) => tag.slice(prefix.length)).filter((n) => /^[1-9]\d*$/.test(n))
    .map(Number)
  if (used.some((n) => !Number.isSafeInteger(n))) throw new Error('Release counter overflow')
  const sequence = Math.max(0, ...used) + 1
  if (!Number.isSafeInteger(sequence)) throw new Error('Release counter overflow')
...
export function replaySeries(repo, base, head, target, destination) {
  for (const sha of [base, head, target]) {
    if (!shaPattern.test(sha)) throw new Error('Replay requires full commit SHAs')
  }
  if (!ancestor(repo, base, head)) throw new Error('downstream-base is not an ancestor of the source')
  if (git(repo, 'rev-list', '--min-parents=2', `${base}..${head}`)) {
    throw new Error('The downstream patch series must be linear: merge commits require manual cleanup')
  }
  if (existsSync(destination)) throw new Error('Refusing to overwrite the candidate directory')
  const commits = git(repo, 'rev-list', '--reverse', `${base}..${head}`).split('\n').filter(Boolean)
  const equivalent = new Set(git(repo, 'cherry', target, head, base).split('\n')
    .filter((line) => line.startsWith('- ')).map((line) => line.slice(2)))
  git(repo, 'worktree', 'add', '--detach', destination, target === base ? head : target)
  const patches = []
  for (const sha of commits) {
    ...
    try {
      ...
      const conflicts = git(destination, 'diff', '--name-only', '--diff-filter=U')
      try { git(destination, 'cherry-pick', '--abort') } catch { /* retain diagnostics */ }
      throw new Error(`Cannot replay ${sha} (${subject}). No patch was silently dropped.\n${conflicts || 'The patch may be partly present or empty; review it manually.'}\n${error.stderr || error.message}`)
    }
    patches.push({ original: sha, result: git(destination, 'rev-parse', 'HEAD'), subject, status: 'replayed' })
  }
  return { sourceSha: git(destination, 'rev-parse', 'HEAD'), patches }
}

export function promotionArgs(plan) {
  for (const field of ['sourceSha', 'previousMain', 'previousBase', 'upstreamSha']) {
    if (!shaPattern.test(plan[field] || '')) throw new Error(`Invalid ${field}`)
  }
  if (!releaseTagPattern.test(plan.version?.tag || '')) throw new Error('Invalid release tag')
  return ['push', '--atomic',
...
function outputs(values) {
  for (const [key, value] of Object.entries(values)) {
    if (/[\r\n]/.test(String(value))) throw new Error('Multiline workflow output')
...
export function prepare() {
  ...
  const previousBase = git(repo, 'rev-parse', 'refs/remotes/origin/downstream-base')
  if (process.env.GITHUB_REPOSITORY !== forkRepository) throw new Error('This workflow is fork-only')
  if (process.env.GITHUB_REF === 'refs/heads/main' && recipeSha !== previousMain) {
    throw new Error('main advanced before preparation; the newer push must be built instead')
  }
  const latest = api(`repos/${upstreamRepository}/releases/latest`)
  if (latest.draft || latest.prerelease || !stableTagPattern.test(latest.tag_name)) {
    throw new Error('Upstream latest is not a supported stable release')
  }
  const upstreamTag = latest.tag_name
  git(repo, 'fetch', '--no-tags', `https://github.com/${upstreamRepository}.git`, `refs/tags/${upstreamTag}`)
  const upstreamSha = git(repo, 'rev-parse', 'FETCH_HEAD^{commit}')
  const baseVersion = JSON.parse(git(repo, 'show', `${previousBase}:package.json`)).version
  const nextCore = upstreamTag.replace(/^v/, '')
  const targetVersion = JSON.parse(git(repo, 'show', `${upstreamSha}:package.json`)).version
  if (targetVersion !== nextCore) throw new Error('Upstream release tag and package version disagree')
  const oldParts = baseVersion.split('.').map(Number)
  const newParts = nextCore.split('.').map(Number)
  for (let i = 0; i < 3; i++) {
    if (newParts[i] > oldParts[i]) break
    if (newParts[i] < oldParts[i]) throw new Error('Refusing an upstream version downgrade')
  }
  const releases = JSON.parse(run('gh', ['api', '--paginate', '--slurp',
    `repos/${forkRepository}/releases?per_page=100`])).flat()
  const published = releases.filter((r) => !r.draft && releaseTagPattern.test(r.tag_name))
  if (baseVersion === nextCore && previousBase !== upstreamSha &&
      published.some((r) => r.tag_name.startsWith(`${upstreamTag}-`))) {
    throw new Error('An already-used upstream release tag moved; review the baseline manually')
  }
  const alreadyBuilt = published.some((r) => {
    if (!r.tag_name.startsWith(`${upstreamTag}-`)) return false
    return git(repo, 'rev-parse', `refs/tags/${r.tag_name}^{commit}`) === recipeSha
  })
  if (previousBase === upstreamSha && alreadyBuilt && process.env.FORCE_BUILD !== 'true') {
  ...
  mkdirSync(out)
  const { sourceSha, patches } = replaySeries(repo, previousBase, recipeSha, upstreamSha, destination)
  const version = nextVersion(upstreamTag, berlinDate(), git(repo, 'tag', '--list').split('\n'))
  const plan = { repository: forkRepository, recipeSha, previousMain, previousBase,
    upstreamTag, upstreamSha, sourceSha, version, patches,
  ...
  git(repo, 'bundle', 'create', join(out, 'source.bundle'), 'refs/heads/downstream-candidate', `^${previousBase}`)
