...
def render_notes(plan, signed=False):
    ...
    for patch in plan.get("patches", []):
        ...
        patches.append(f"- {status}: `{label}` ([`{sha[:8]}`](https://github.com/{REPOSITORY}/commit/{sha}))")
    distribution = (
    ...
    return (
        ...
        "### Included changes\n\n" + ("\n".join(patches) or "No additional application patches.") + "\n\n"
        ...
        "Felix previously reported working stacking and Nile login in a custom Linux build.\n"
    )
