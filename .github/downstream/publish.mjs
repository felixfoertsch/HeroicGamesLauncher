...
if (plan.sourceSha !== process.env.EXPECTED_SOURCE_SHA)
  throw new Error('Unexpected candidate source')

function gh(...args) {
  return run('gh', [...args, '--repo', forkRepository])
}
  ...
  return readdirSync(assets)
    ...
    .map((name) => {
      if (
        ...
        !lstatSync(join(assets, name)).isFile()
      ) {
...
if (mode === 'promote') {
  ...
  git(
    repo,
    'fetch',
    resolve('plan/source.bundle'),
    'refs/heads/downstream-candidate:refs/heads/downstream-candidate'
  )
  if (
    git(repo, 'rev-parse', 'refs/heads/downstream-candidate') !== plan.sourceSha
  ) {
    throw new Error('Source bundle does not match the tested commit')
  }
  if (currentMain() !== plan.previousMain)
    throw new Error('main changed while building; nothing will be published')
  run('gh', ['auth', 'setup-git'])
  git(repo, ...promotionArgs(plan))
} else if (mode === 'release') {
  if (currentMain() !== plan.sourceSha)
    throw new Error(
      'main advanced after promotion; refusing to update the feed'
    )
  ...
  gh(
    ...
    '--draft',
    '--title',
    `Heroic ${plan.version.tag} (Felix downstream)`,
    '--notes-file',
    join(assets, 'RELEASE-NOTES.md')
  )
  ...
  if (!releases.some((r) => r.tag_name === 'downstream-feed')) {
    gh(
      ...
      plan.sourceSha,
      '--title',
      'Downstream update feed (not an application version)',
      '--notes',
      'Mutable AppImage and signed pacman repository metadata. Versioned application releases remain immutable.',
      '--prerelease',
      '--latest=false'
    )
  }
  ...
  if (existsSync(join(assets, 'heroic-felix.db'))) {
    const packages = releaseFiles().filter((p) =>
      /\.pkg\.tar\.zst(?:\.sig)?$/.test(p)
    )
    gh('release', 'upload', 'downstream-feed', ...packages)
    for (const name of [
      'heroic-felix.asc',
      'heroic-felix.fingerprint',
      'heroic-felix.db',
      'heroic-felix.db.sig',
      'heroic-felix.files',
      'heroic-felix.files.sig'
    ]) {
      gh(
        'release',
        'upload',
    ...
    console.log(
      'Signed pacman feed not updated: configure PACMAN_SIGNING_KEY and PACMAN_SIGNING_FINGERPRINT.'
    )
  }
  gh(
    'release',
  ...
  gh(
    ...
    'downstream-feed',
    '--notes',
    `Mutable downstream update metadata. Application releases remain immutable.\n\nsource_sha=${plan.sourceSha}\nrelease_tag=${plan.version.tag}\n`
  )
  console.log(
    `Published https://github.com/${forkRepository}/releases/tag/${plan.version.tag}`
  )
