...
export function stage(source, plan) {
  if (
    plan.repository !== forkRepository ||
    !releaseTagPattern.test(plan.version.tag)
  ) {
    throw new Error('Invalid release plan')
  }
  if (git(source, 'rev-parse', 'HEAD') !== plan.sourceSha)
    throw new Error('Wrong source checkout')
...
export function updateMetadata(plan, imagePath) {
  ...
  return (
    `version: ${JSON.stringify(plan.version.electron)}\nfiles:\n` +
...
export function collect(source, plan, out) {
  ...
  const names = readdirSync(dist).filter(
    (name) =>
      name.startsWith(`Heroic-${plan.version.tag}-`) &&
      (name.endsWith('.AppImage') || name.endsWith('.tar.xz'))
  )
  if (
    names.filter((n) => n.endsWith('.AppImage')).length !== 1 ||
  ...
  writeFileSync(
    join(out, 'latest-linux.yml'),
    updateMetadata(plan, join(out, image))
  )
  writeJson(join(out, 'build-info.json'), plan)
  writeFileSync(
    join(out, 'RELEASE-NOTES.md'),
    `# Heroic ${plan.version.tag}\n\n` +
      ...
      plan.patches
        .map(
          (p) => `- ${p.subject} — ${p.status} (\`${p.result || p.original}\`)`
        )
        .join('\n') +
  ...
  for (const name of [
    'package.json',
    'electron-builder.downstream.json',
    'downstream-build-info.json'
  ]) {
    cpSync(join(source, name), join(archiveDir, name))
  }
  run('tar', [
    '-czf',
    join(out, `Heroic-${plan.version.tag}-source.tar.gz`),
    '-C',
    archiveDir,
    '.'
  ])
...
export function writePkgbuild(plan, archive, directory) {
  ...
  writeFileSync(
    ...
    template
      .replaceAll('@PKGVER@', plan.version.pacman)
