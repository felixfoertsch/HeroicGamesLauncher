...
describe('getGameLibraryStats', () => {
  ...
  it('reconciles 400 Epic and 100 Amazon copies with 10 cross-store duplicates', () => {
    const epic = Array.from({ length: 400 }, (_, i) =>
      game('legendary', `e${i}`, `Epic ${i}`)
    )
    const amazon = Array.from({ length: 100 }, (_, i) =>
      game('nile', `a${i}`, i < 10 ? `Epic ${i}` : `Amazon ${i}`)
    )
  ...
  it('counts only copies supplied by the current store, search or visibility filters', () => {
    const library = [
      game('legendary', 'same-id'),
      game('gog', 'same-id'),
      game('gog', 'other', 'Other')
    ]
    const selected = library.filter(
      (item) => item.runner === 'gog' && item.title === 'Example'
    )
  ...
  it('does not count an uninstalled copy excluded from the current view', () => {
    const library = [
      game('legendary', 'e'),
      game('gog', 'g', 'Example', { is_installed: true })
    ]
    expect(
      getGameLibraryStats(library.filter((item) => item.is_installed))
    ).toMatchObject({
      games: 1,
      copies: 1,
      extraCopies: 0,
  ...
  it('keeps editions and ambiguous same-store matches separate', () => {
    const library = [
      ...
      game('gog', 'g1'),
      game('gog', 'g2'),
      game('nile', 'remaster', 'Example Remastered')
    ]
    expect(getGameLibraryStats(library)).toMatchObject({
      games: 4,
      copies: 4,
      multiStoreGames: 0,
      extraCopies: 0
  ...
  it('supports Zoom while treating sideloads as independent sources', () => {
    expect(
      getGameLibraryStats([
        game('gog', 'g'),
        game('zoom', 'z'),
        game('sideload', 's', 'Example', { install: { is_dlc: true } })
      ])
    ).toEqual({
      games: 2,
      copies: 3,
  ...
  it('uses the same normalized store titles as the rendered stacks', () => {
    const library = [
      game('legendary', 'e', '  GAME: Part-One™ '),
      game('gog', 'g', 'Game – Part One'),
      game('nile', 'other', 'Other', {
        overrides: { title: 'Game – Part One' }
      })
    ]
    expect(getGameLibraryStats(library)).toMatchObject({
      games: 2,
  ...
  it('returns zeroes for an empty view', () => {
    expect(getGameLibraryStats([])).toEqual({
      games: 0,
      copies: 0,
      multiStoreGames: 0,
      extraCopies: 0,
      copiesByRunner: emptySources
    })
  })
  ...
  it('does not mutate records and reconciles every total with the card groups', () => {
    ...
    expect(stats.copies).toBe(
      Object.values(stats.copiesByRunner).reduce((sum, value) => sum + value, 0)
    )
