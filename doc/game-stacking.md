...
The store logo identifies that representative. Running games and downloads
therefore remain accessible without treating different installations as one.
...
5. Install or launch the non-representative copy from its own page, then return
   to the library. Verify its progress/stop actions and lazy-loaded artwork,
   including after the operation completes and the representative changes.
